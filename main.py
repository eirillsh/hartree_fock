...
def plot_MO_energies(E, atom, Z):
	...
	plt.xticks(x, [r"$\psi_%d$" %i for i in x], fontsize=17)
	plt.yticks(y_tick, ["%.3f" %y for y in y_tick], fontsize=15)
	plt.grid()
	plt.xlabel(r"$\psi_i$", fontsize=20)
	plt.ylabel(r"MO energy [a.u]", fontsize=20)
	plt.tight_layout()
...
def plot_AO_energies(basis, atom):
	...
	plt.yticks(y_tick, ["%.3f" %y for y in y_tick], fontsize=15)
	plt.grid()
	plt.xlabel(r"$\phi_\alpha$", fontsize=20)
	if atom == "H":
		plt.ylabel(r"AO energy $\frac{\mathrm{[a.u]}}{Z^2}$", fontsize=20)
	else:
		plt.ylabel(r"AO energy [a.u]", fontsize=20)
	plt.tight_layout()
